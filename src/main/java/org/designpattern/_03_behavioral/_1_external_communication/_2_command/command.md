...
## 2. 구조
![img.png](img.png)
- 출처 : https://www.softwareideas.net/a/460/command-design-pattern-uml-diagrams-

### 1) Command (추상 커맨드)
- 요청을 캡슐화하기 위한 인터페이스로, execute() 메서드를 정의


### 2) ConcreteCommand (구체 커맨드)
- Command를 구현하며, 요청을 처리하기 위한 로직을 포함
- Receiver를 호출하여 실제 작업을 수행


### 3) Receiver (수신자)
- 요청을 처리하는 실제 작업을 수행하는 객체입니다.


### 4) Invoker (호출자)
- Command 객체를 실행하며, 요청을 큐에 저장하거나 실행 취소를 수행할 수 있습니다.


### cf) Client (클라이언트)
- ConcreteCommand 객체를 생성하고, Receiver와 연결
- Invoker에 Command를 설정하여 요청을 실행


## 3. 정의
-  [요청을 객체로 캡슐화]하여, 요청을 매개변수화, 로깅, 실행 취소할 수 있는 디자인 패턴

## 4. 목적
### 1) 요청 캡슐화
- 요청을 Command 객체로 캡슐화하여, 호출자와 수신자 간의 의존성을 줄임
### 2) 작업 큐 지원
- 요청을 큐에 저장하고, 나중에 실행하거나 실행 취소 가능
### 3) 확장성 증가
- 새로운 명령을 추가해도 기존 코드 수정 없이 확장 가능
